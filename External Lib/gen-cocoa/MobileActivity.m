...
- (void) write: (id <TProtocol>) outProtocol {
  ...
  if (__metaData_isset) {
    if (__metaData != nil) {
      [outProtocol writeFieldBeginWithName: @"metaData" type: TType_MAP fieldID: 3];
      {
        [outProtocol writeMapBeginWithKeyType: TType_STRING valueType: TType_STRING size: [__metaData count]];
        NSEnumerator * _iter4 = [__metaData keyEnumerator];
        id key5;
        while ((key5 = [_iter4 nextObject]))
        {
          [outProtocol writeString: key5];
          [outProtocol writeString: [__metaData objectForKey: key5]];
        }
